...
if (empty($_GET['roll_no']) && $_GET['course_id'] && $_GET['semester']) {
...
if (isset($_POST['submit'])) {
    ...
    $lst_sql ="insert into tb_last_result(sid,semester,course_name,course_id) Values('$sid','$semester','$c_name','$course_name')";
    ...
    $stmt = $conn->prepare("INSERT INTO tb_results (sid, course_name, semester, subject, maximum, minimum, external) VALUES (?, ?, ?, ?, ?, ?, ?)");
    ...
    for ($i = 0; $i < count($subjects); $i++) {
        ...
        $stmt->bind_param("isissii", $sid, $course_name, $semester, $subject, $maximum, $minimum, $external);
        $stmt->execute();
    }
