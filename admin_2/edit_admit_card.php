...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $sql = "UPDATE admit_cards SET 
            course = '$course', exam = '$exam', exam_date = '$date', 
            recept_no = '$recept_no', exam_center_address = '$exam_center_address', 
            name = '$name', dob = '$dob', contact = '$contact', address = '$address'
            WHERE id = $id";
    
    if (mysqli_query($conn, $sql)) {
        $message = "Admit card updated successfully!";
        // Redirect to manage_admit_cards.php after update
        header("Location: manage_admit_cards.php");
        exit();
    } else {
        $message = "Error updating admit card: " . mysqli_error($conn);
    }
}
?>
        ...
        <form action="edit_admit_card.php?id=<?= $id ?>" method="POST">
            ...
            <div class="row">
                <div class="form-group col-lg-6">
                    ...
                    <input type="date" class="form-control" id="date" name="exam_date" value="<?= $admitCard['exam_date'] ?>" required>
                </div>
                <div class="form-group col-lg-6">
